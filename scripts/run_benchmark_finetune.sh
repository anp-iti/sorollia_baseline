...
# Example:
# FINETUNE_ARGS="--model_type=Transfer_Cnn14 --batch_size=32 --learning_rate=1e-3 --cuda"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            echo "Usage:"
            echo "  bash scripts/run_benchmark_finetune.sh --fold-group [cv|non_cv|both]"
            echo ""
            echo "Examples:"
            echo "  bash scripts/run_benchmark_finetune.sh --fold-group cv"
            echo "  bash scripts/run_benchmark_finetune.sh --fold-group non_cv"
            echo "  bash scripts/run_benchmark_finetune.sh --fold-group both"
            echo ""
...
if [ ! -f "${FINETUNE_SCRIPT}" ]; then
    echo "ERROR: Fine-tuning script not found: ${FINETUNE_SCRIPT}"
    exit 1
fi
...
echo ""
echo "============================================================"
echo "Running fine-tuning benchmark"
...
for FOLD_GROUP in "${FOLD_GROUPS[@]}"; do
    ...
    for TEST_FOLD in 0 1 2 3 4; do
        ...
        echo ""
        echo "[OK] Finished fine-tuning ${FOLD_GROUP} fold_${TEST_FOLD}"
    done
done

echo ""
echo "============================================================"
echo "Fine-tuning benchmark completed successfully."
echo "============================================================"