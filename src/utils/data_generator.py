...
class BalancedTrainSampler(Base):
    def __init__(self, indexes_hdf5_path, batch_size, black_list_csv=None, 
        ...
        logging.info('Number of samples per class: '+'\n'.join(f'Class {cl}: {sample}' for sample, cl in zip(self.samples_num_per_class, self.labels)))
    ...
    def __iter__(self):
        ...
        while True:
            ...
            while i < batch_size:
                ...
                if len(self.indexes_per_class[class_id]) > 0: # If the class is not appearing in the dataset, then do not do anything, just discard the class.
